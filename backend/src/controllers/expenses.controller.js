...
export async function addExpense(req, res) {
  ...
  if (!groupId || !description || !Number.isFinite(amt) || amt <= 0 || !paidBy || !Array.isArray(splitAmong) || splitAmong.length === 0) {
  ...
  await Promise.all([
    ...
    ...splitAmong.map(uid => Balance.updateOne(
      { groupId, userId: uid },
      { $inc: { balance: -share } },
      { upsert: true }
    ))
...
export async function getGroupExpenses(req, res) {
  ...
  const userIds = Array.from(new Set(expenses.flatMap(e => [e.paidBy, ...e.splitAmong])));
...
export async function getBalances(req, res) {
  ...
  const users = await User.find({ _id: { $in: balances.map(b => b.userId) }}).select("_id name email").lean();
